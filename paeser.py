...
def save_to_json(data: List[Dict[str, Any]], filename: str = OUTPUT_FILE) -> None:
...
async def main() -> None:
    ...
    async with OzonScraper(url) as scraper:
        ...
        # Конвертируем список ProductInfo в список словарей
        cards_data = [card.to_dict() for card in search_cards]

        # Сохраняем в JSON файл
        save_to_json(cards_data)

        print(f"Успешно собрано {len(cards_data)} товаров")
